...
const UserLogin = () => {
  ...
  return (
    <div className='h-screen w-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center p-3 sm:p-4 relative overflow-hidden'>
      
      {/* Animated background elements */}
      <div className='absolute top-0 left-0 w-72 h-72 bg-blue-300 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-blob pointer-events-none'></div>
      ...
      {/* Logo - Top Left */}
      <Link to="/" className="fixed top-3 left-3 sm:top-4 sm:left-4 z-50 group">
        <img 
          src={logo1} 
          alt="EasyGo" 
      ...
      <div className='relative w-full max-w-md z-10'>
        <div className='bg-white/90 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-2xl p-5 sm:p-6 md:p-8 border border-white/50 transform transition-all duration-300'>
          ...
          <div className='text-center mb-5 sm:mb-6'>
            <div className='inline-flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl sm:rounded-2xl mb-3 shadow-lg'>
              <i className="ri-user-line text-2xl sm:text-3xl text-white"></i>
            </div>
            <h2 className='text-xl sm:text-2xl font-bold text-gray-800 mb-1'>Welcome back</h2>
            <p className='text-xs sm:text-sm text-gray-600'>Sign in to continue</p>
          </div>
          ...
          <form onSubmit={submitHandler} className='space-y-3 sm:space-y-4'>
            <div>
              ...
              <div className='relative'>
                ...
                <input
                  ...
                  className='w-full rounded-lg sm:rounded-xl pl-10 sm:pl-12 pr-3 sm:pr-4 py-2.5 sm:py-3 bg-gray-50 border-2 border-gray-200 text-sm sm:text-base focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all duration-200'
                  type="email"
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className='w-full py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-white font-semibold text-sm sm:text-base bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2'
            >
              {isLoading ? (
                <>
          ...
          <div className='space-y-3'>
            <p className='text-center text-xs sm:text-sm text-gray-600'>
              New to EasyGo? 
              <Link to='/signup' className='ml-1 text-indigo-600 font-semibold hover:text-indigo-700 hover:underline'>
                Create account
              </Link>
            </p>
